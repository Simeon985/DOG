...
def locations_Kalman(data, scale_factor_1, scale_factor_2, angle_1, angle_2):
    ...
    class PeripheralEstimator:
        ...
        # def update_without_imu(self, dt, flow, heading=None):
        #     """
        #     flow = [deltaX1, deltaY1, deltaX2, deltaY2]
        #     Using R1, R2 and calculated geometry to find omega.
        #     """
        #     # 1. Calculate local velocities (Your existing logic)
        #     fx1, fy1, fx2, fy2 = flow

        #     v_x1 = (fx1*self.angle_1_cos + fy1*self.angle_1_sin) / dt
        #     v_y1 = (- fx1*self.angle_1_sin + fy1*self.angle_1_cos) / dt

        #     v_x2 = (fx2*self.angle_2_cos + fy2*self.angle_2_sin) / dt
        #     v_y2 = (- fx2*self.angle_2_sin + fy2*self.angle_2_cos) / dt

        #     # 2. Calculate Angular Velocity (Omega)
        #     # Rotation causes a difference in velocities proportional to the distance from center.
        #     # We use the difference in 'tangential' components.
        #     # Formula: omega = (v_sensor2 - v_sensor1) / (dist2 - dist1)
        #     # Given your asymmetric mounting, we use the average delta:

        #     # We solve for omega: v_tangential = omega * R
        #     # Using the difference between the two sensors to cancel out common linear motion:
        #     numerator = (v_y2 - v_y1)
        #     T_X_1, T_Y_1 = 0.0916, .0514
        #     self.R_1 = np.sqrt(T_X_1**2 + T_Y_1**2)
        #     T_X_2, T_Y_2 = -0.0730, -0.0739
        #     self.R_2 = np.sqrt(T_X_2**2 + T_Y_2**2)
        #     denominator = (self.R_1 + self.R_2) # Combined leverage arm

        #     omega = numerator / denominator

        #     # 3. Update Heading (Integration)
        #     self.pose[2] += omega * dt
        #     p_a = self.pose[2]

        #     # 4. Average linear velocities for position
        #     v_x = (v_x1 + v_x2) / 2.0
        #     v_y = (v_y1 + v_y2) / 2.0
...
def plot_trajectories(data):
    ...
    plt.plot(x_peri, y_peri, 'b-', linewidth=2, label='Peripheral')
    plt.scatter(x_peri[0], y_peri[0], c='green', label='Start')
    plt.scatter(x_peri[-1], y_peri[-1], c='blue', label='End')
...
def main(filename, recalibrate):
    ...
    scale_factor_1, scale_factor_2, angle_1, angle_2 = 1,1,0,0
    if recalibrate:
